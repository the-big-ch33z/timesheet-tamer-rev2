...
const WorkHoursDisplay: React.FC<WorkHoursDisplayProps> = ({
  ...
  const breakElements = useMemo(() => {
    ...
    if (breaksIncluded.lunch) {
      elements.push(<span key="lunch" className="flex items-center px-[0.5em] py-[0.15em] rounded-full bg-lime-50 border border-lime-200 text-lime-700 text-xs">
          <Bell className="h-3 w-3 mr-1" />
          Lunch subtracted
        </span>);
    }
    if (overrideStates.lunch) {
      elements.push(<span key="lunch-override" className="flex items-center px-[0.5em] py-[0.15em] rounded-full bg-blue-50 border border-blue-200 text-blue-700 text-xs">
          <Bell className="h-3 w-3 mr-1" />
          Lunch override
        </span>);
    }
    if (breaksIncluded.smoko) {
      elements.push(<span key="smoko" className="flex items-center px-[0.5em] py-[0.15em] rounded-full bg-yellow-50 border border-yellow-200 text-yellow-800 text-xs">
  ...
  return <div className="w-full p-4 bg-white border-t border-gray-200 rounded-b-md">
      <div className="flex items-center justify-start gap-6 mb-2">
        <div className="flex flex-col">
          <TimeInput id="start-time" label="Start Time" value={startTime} onChange={handleStartTimeChange} disabled={!interactive} className="w-32" placeholder="Enter start time" debounceMs={300} />
        </div>

        <div className="flex flex-col">
          <TimeInput id="end-time" label="End Time" value={endTime} onChange={handleEndTimeChange} disabled={!interactive} className="w-32" placeholder="Enter end time" debounceMs={300} />
        </div>
        ...
        {showSaved && <div className="flex items-center text-green-600 text-xs animate-fade-in">
