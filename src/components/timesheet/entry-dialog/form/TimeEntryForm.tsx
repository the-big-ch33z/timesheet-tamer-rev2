        ...
        <div className="space-y-4">
          <div className="flex flex-col md:flex-row gap-4">
            <div className="w-full md:w-24">
              <HoursField id="hours" value={formState.fields.hours.value} onChange={value => setFieldValue(FIELD_TYPES.HOURS, value)} required={true} />
              {formState.fields.hours.error && <p className="text-red-500 text-sm mt-1">{formState.fields.hours.error}</p>}
            </div>
            
            <div className="w-full md:w-32">
              {renderFormField(FIELD_TYPES.JOB_NUMBER, formState.fields.jobNumber.value, value => setFieldValue(FIELD_TYPES.JOB_NUMBER, value), false, formState.fields.jobNumber.error)}
            </div>
            
            <div className="w-full md:w-24">
              {renderFormField(FIELD_TYPES.REGO, formState.fields.rego.value, value => setFieldValue(FIELD_TYPES.REGO, value), false, formState.fields.rego.error)}
            </div>
            
            <div className="w-full md:w-32">
              {renderFormField(FIELD_TYPES.TASK_NUMBER, formState.fields.taskNumber.value, value => setFieldValue(FIELD_TYPES.TASK_NUMBER, value), false, formState.fields.taskNumber.error)}
            </div>
          </div>
          
          {showTimeInputs && <TimeFields startTime={formState.fields.startTime.value} endTime={formState.fields.endTime.value} setStartTime={value => setFieldValue(FIELD_TYPES.START_TIME, value)} setEndTime={value => setFieldValue(FIELD_TYPES.END_TIME, value)} selectedDate={date} disabled={isLoading} />}
          
          <div>
            <label className="block text-sm font-medium mb-1">Description</label>
            <Textarea value={formState.fields.description.value} onChange={e => setFieldValue(FIELD_TYPES.DESCRIPTION, e.target.value)} placeholder="Enter description" className="w-full" rows={2} />
          </div>
